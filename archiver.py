...
def main():
    ...
    if args.file:
        ...
        with open(args.file) as file:
            urls_from_file = (u.strip() for u in file.readlines() if u.strip() if not u.startswith("//"))
    ...
    if args.file:
        logging.info("Cleaning urls from file: %s", args.file)
        with open(args.file, mode="r+") as file:
            content = file.read()
            with open("archived_" + args.file, "a") as file2:
                file2.write(content)
            file.seek(0)
            file.truncate()
